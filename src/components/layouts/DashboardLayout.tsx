    ...
    return (
      <Link 
        ...
        className={cn(
          ...
          isActive 
            ? "bg-eigengram-primary text-white" 
  ...
  const NavContent = () => (
    <>
      <div className="px-4 py-6">
        <h2 className="text-xl font-bold text-gradient">Eigengram</h2>
        <p className="mt-1 text-sm text-eigengram-muted">Healthcare AI Platform</p>
      </div>
      <Separator className="mb-4" />
      <nav className="space-y-1 px-2">
        <NavItem to="/dashboard" icon={Home} label="Dashboard" />
        <NavItem to="/dashboard/services" icon={Search} label="Search Services" />
        <NavItem to="/dashboard/subscriptions" icon={BookOpen} label="My Subscriptions" />
      ...
      <div className="mt-auto px-4 py-4">
        {user?.role === "ADMIN" && (
          ...
          >
            <span className="text-eigengram-accent font-medium">Admin Dashboard</span>
          </Link>
        )}
        <Separator className="mb-4" />
        <Button 
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="bg-white border-b border-eigengram-border py-4 px-6 flex items-center justify-between">
          ...
          <div className="flex-1 flex justify-center md:justify-start">
            {isMobile && <h1 className="text-xl font-bold text-gradient">Eigengram</h1>}
          </div>
